...
def get_pitch(audio):
    pitch_mel, voiced_flag, voiced_probs = librosa.pyin(
        audio, fmin=55.,
        fmax=7500., frame_length=640)
    pitch_mel = np.where(np.isnan(pitch_mel), 0.0, pitch_mel)
    ii = pitch_mel != 0.
    mean_f0 = np.mean(pitch_mel[ii])
    std_f0 = np.std(pitch_mel[ii])
    f0_norm = (pitch_mel - mean_f0) / (std_f0 + 1e-8)
    f0_norm[~ii] = 0.
    f0_norm = f0_norm[::4]
    return f0_norm

def pitch_norm(pitch):
    ii = pitch != 0.
    mean_f0 = np.mean(pitch[ii])
    std_f0 = np.std(pitch[ii])
    f0_norm = (pitch - mean_f0) / (std_f0 + 1e-8)
    f0_norm[~ii] = 0.
    return f0_norm
...
def plot_curves(c_name1, c1, c_name2, c2):
    c1 = to_numpy(c1)
    c2 = to_numpy(c2)
    fig, ax = plt.subplots(figsize=(10, 4))
    ax.plot(c1, label=c_name1)
    ax.plot(c2, label=c_name2)
    ax.set_xlabel('time')
    ax.set_ylabel('value')
    plt.tight_layout()   
    fig.canvas.draw()
