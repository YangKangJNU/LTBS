...
class FlowSpecDecoder(nn.Module):
  def __init__(self, cfg):
    ...
    self.n_sqz = cfg.n_sqz
    self.sigmoid_scale = cfg.sigmoid_scale
    self.gin_channels = cfg.gin_channels

    self.flows = nn.ModuleList()
    for b in range(self.n_blocks):
      self.flows.append(ActNorm(channels=self.in_channels * self.n_sqz))
      self.flows.append(InvConvNear(channels=self.in_channels * self.n_sqz, n_split=self.n_split))
      self.flows.append(
        CouplingBlock(
    ...
    # train: gen=False, reverse=Fasle, y=mel_gt, g=spk_emb, 
    # eval: gen=True, reverse=True
  def forward(self, x, x_mask, g=None, reverse=False):
    ...
    if self.n_sqz > 1:
      x, g, x_mask = squeeze_with_cond(x, g, x_mask, self.n_sqz)
    for f in flows:
      if not reverse:
...
def squeeze_with_cond(x, g, x_mask=None, n_sqz=2):
  b, c, t = x.size()
  ...
  g_sqz = g.view(b, g.size(1), t//n_sqz, n_sqz)
  g_sqz = g_sqz.permute(0, 3, 1, 2).contiguous().view(b, g.size(1)*n_sqz, t//n_sqz)
  
  if x_mask is not None:
    x_mask = x_mask[:,:,n_sqz-1::n_sqz]
  else:
    x_mask = torch.ones(b, 1, t//n_sqz).to(device=x.device, dtype=x.dtype)
  return x_sqz * x_mask, g_sqz * x_mask, x_mask
